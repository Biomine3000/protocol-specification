...
** Routing
   ...
   There are now two conflicting solutions for automatically providing 
   routing id:s (hopefully we can come to an agreement later):

   leronen:
   A client had better first request (or provide) a routing
   id before sending any other objects. If this is not
   done, the server will automatically provide one when
   receiving the first object from the client. Until
   a client has sent its first object (either implicitly
   or explicitly subscribing routing), the connection
   is not yet considered fully formed, and nothing is sent 
   to the client.
    
   guaqua: (added by leronen, please edit if this does 
   not represent the said pundit's true vision)
   A client is immediately assigned a routing id upon
   TCP connect, and starts receiving content without 
   further ado. The client can later change the assigned
   by a routing/subscribe request if needed (a subscription
   does not change the id unless a new id is provided by the 
   client).
